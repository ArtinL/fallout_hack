...
HackGame::HackGame(bool dbg, bool dly) : debug(dbg), delay(dly), errormsg("") {
    ...
    system("clear");
    setup_hack(selectedDifficulty);
}

void HackGame::setup_hack(int diff) {
    errormsg.clear();
    ...
    words.clear();
    populateWords(words, chosenLength);
    ...
    chosenWords.clear();
    correctWord = chooseWords(words, chosenWords, numWords);
    ...
    interlaceWords(chosenWords, chosenLength);
    fillChars();
    findCheats();
}
...
int HackGame::hack() {
    ...
    //std::vector<std::string> triedCheats;
    
    int startHex = rand() % 0x10000; 
    int iter = 0;

    while (allowedAttempts > 0) {
        drawGrid((delay && iter == 0), startHex, allowedAttempts);
        drawPrompts(triedWords, chosenLength);
        if (gameWon) break;
        
        std::string word;
        InputResult result = detect_input(chosenWords, word, triedWords);

        TriedWord triedWord;
        triedWord.word = word;
        
        if (result == REMOVE_DUD) {
            if (!removeDud(chosenWords, correctWord)) {
                result = REPLENISH_TRIES;
            }
        }
        if (result == REPLENISH_TRIES) {
            //triedCheats.push_back(word);
            allowedAttempts = 4;
        }
        triedWord.type = result;
        
        triedWord.likeness = likeness(word, correctWord);
        if (triedWord.likeness == chosenLength) gameWon = true;
        
        triedWords.push_back(triedWord);

        iter++;
        if (!gameWon && result == VALID_WORD) allowedAttempts--;
    }
...
void HackGame::populateWords(std::vector<std::string>& words, int chosenLength) {
...
std::string HackGame::chooseWords(std::vector<std::string>& words, std::vector<std::string>& chosenWords, int numWords) {
...
void HackGame::interlaceWords(std::vector<std::string>& chosenWords, int wordLength) {
    int gridSize = 36 * 16; 
    int numWords = chosenWords.size();
    int totalSpacing = gridSize - numWords * wordLength; 
...
void HackGame::drawGrid(bool useDelay, int startHex, int allowedAttempts) {
    system("clear");

    delay_printf(useDelay, "ROBCO INDUSTRIES (TM) TERMLINK PROTOCOL\n");
    delay_printf(useDelay, "Enter password now\n\n");
    delay_printf(useDelay, "%d ATTEMPT(S) REMAINING: ", allowedAttempts);
    for (int i = 0; i < allowedAttempts; i++) {
        delay_printf(useDelay, "\u25A0 ");
    }
    delay_printf(useDelay, "\n\n");
    ...
    for (int i = 0; i < 36; i++) {
        ...
        delay_printf(useDelay, "0x%04X ", startHex + i * 16);
        for (int j = 0; j < 16; j++) {
            delay_printf(useDelay && (j % 5 == 0), "%c", grid[i][j]);
        }
    }
}

void HackGame::drawPrompts(std::vector<TriedWord>& triedWords, int chosenLength) {
    int coord2Y = 23;
    bool won = false;

    for (int i = triedWords.size() - 1; i >= 0 && coord2Y >= 7; i--) {
        TriedWord& word = triedWords[i];
...
InputResult HackGame::detect_input(std::vector<std::string>& chosenWords, std::string& input, 
                          std::vector<TriedWord>& triedWords) {
    char tempInput[16];
    std::cin >> tempInput;
    input = tempInput;
    ...
    // if input contains special characters, check for cheats. otherwise, check for valid words
...
bool HackGame::removeDud(std::vector<std::string>& chosenWords, const std::string& correctWord) {
...
void HackGame::displaySuccess(int chosenLength) {
    struct winsize w;
